...
if uploaded_file is not None:
    try:
        ...
        st.success(f"선택한 항목에서 추출된 '{col1_clean}', '{col2_clean}'가 열 제목으로 반영됩니다!")

    except Exception as e:
        st.error("⚠️ 처리 중 오류가 발생했습니다. 파일 구조나 형식을 확인해 주세요.")
        # 오류 메시지를 숨기되 로그에는 남기고 싶다면:
        # st.exception(e)  ← 이것도 주석 처리하면 사용자는 에러 내용을 못 봅니다.
else:
